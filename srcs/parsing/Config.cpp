...
Config&	Config::operator=(const Config& rhs) {
	...
	t_map_it< std::string, std::map< int, std::map< std::string, Rules* > > >::t
		hostIt = parsedConfigRef.begin(), hostIte = parsedConfigRef.end();
	while (hostIt != hostIte) {
		std::string host = hostIt->first;
		t_map_it< int, std::map< std::string, Rules* > >::t
			portIt = hostIt->second.begin(), portIte = hostIte->second.end();
			while (portIt != portIte) {
				...
				while (sNamesIt != sNamesIte) {
					...
					_parsedConfig[host][port][serverName] = new Rules(*(parsedConfigRef.at(host).at(port).at(serverName)));
					sNamesIt++;
				}
				portIt++;
			}
